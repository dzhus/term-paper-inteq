...
\begin{equation}
  \label{eq:gs-series}
  g(x) = \sum_{k=1}^{\infty}a_k\psi_k(x)
\end{equation}
...
функций:
\begin{equation}
  K(x, t) = \sum_{k=1}^{\infty}{\Bigl [ \smallint_a^b{K(x, t)\psi_k(t)\,
      dt} \Bigr ] \psi_k(t)}
\end{equation}
...
\begin{equation}
  ...
  K(x, t) = \sum_{k=1}^{\infty}{\frac{\psi_k(x)\psi_k(t)}{\lambda_k}}
\end{equation}
...
\begin{equation}
  \label{eq:gs-step2}
  \phi(x)-f(x) = \sum_{k=1}^{\infty}{a_k \psi_k(x)}
\end{equation}
...
\begin{multline}
  ...
  \lambda \intl_a^b\sum_{k=1}^\infty{\frac{\psi_k(x)\psi_k(t)}{\lambda_k}\phi(t)\,dt} =\\
  = \lambda \sum_{k=1}^\infty \biggl [ \intl_a^b{\phi(t)\psi_k(t)\,dt}
  \biggr ] \frac{\psi_k(x)}{\lambda_k} = \lambda \sum_{k=1}^\infty
...
\begin{equation}
  ...
  \phi(x) = f(x) + \lambda\sum_{k=1}^\infty{\frac{f_k \psi_k(x)}{\lambda_k-\lambda}}
\end{equation}

Заметим, что для справедливости \eqref{eq:gs-solution} параметр
...
\eqref{eq:gs-solution} требуется:
\begin{enumerate}
...
Для решения данной краевой задачи рассмотрим три варианта значений
характеристических чисел $\lambda_k$.
\begin{enumerate}
\item $\lambda=1$

  Присвоим такое значение характеристическому числу $\lambda_1$.
  ...
  Из краевых условий получаем $C_1=0 \implies \psi_1(x) = C_2$, в целях
  нормировки выберем значение $C_2 = 1$.

  Итак, характеристическому значению $\lambda_1=1$ соответствует
...
\item $\lambda > 1$
  ...
  Обозначим $\lambda-1 = h^2 > 0$, получив уравнение
  \begin{equation*}
  ...
  собственных функций. Не ограничивая общности, для определённости
  положим $n = k$.

  С учётом выполненной замены $h^2 = \lambda - 1$, получим систему
  ...
  Таким образом, получена ортонормированная система собственных
  функций и характеристических чисел
  \begin{align}
...
\item $\lambda < 1$
  ...
  Вводя обозначение $1 - \lambda = h^2$, получим уравнение
  \begin{equation*}
    \psi_k''(x) - h^2 \psi_k(x) = 0
  \end{equation*}
  ...
  Поскольку $C_1 ≠ 0$, получаем $h = 0 \implies \lambda = 1$, что
  противоречит предположению $\lambda < 1$. Значит, в данной области
  у рассматриваемого интегрального оператора характеристических
  значений нет.
\end{enumerate}

С учётом полученных результатов заключаем, что данное по условию
значение параметра $\lambda = 2$ не принадлежит $\{\lambda_k\}$, так что
формула \eqref{eq:gs-solution} остаётся справедливой.

\subsubsection{Вычисление коэффициентов ряда}

Для построения ряда \eqref{eq:gs-solution} необходимо определить
значения коэффициентов $f_k$.

Первый коэффициент вычисляется элементарно:
\begin{equation}
  f_1 = \intl_0^1 \ch{x} \,dx = \sh{1}
\end{equation}

Для вычисления коэффициентов $f_2, f_3, \dotsc$ найдём значение
...
Таким образом, коэффициенты $f_k$ для $k = 2, 3, \dotsc$ равны
\begin{equation}
  f_k = \frac{\sqrt{2} \cdot \sh{1}\cdot (-1)^k}{\pi^2 k^2 + 1}
\end{equation}

Подставляя значения $f_k, \lambda, \lambda_k, \psi_k$ в
\eqref{eq:gs-solution}, получим решение уравнения в виде ряда по
системе функций $\{\psi_k\}$:
\begin{multline}
  \phi(x) = f(x) + 2 \sum_{k=1}^\infty \left [
    f_k\cdot\frac{\psi_k(x)}{\lambda_k-2} \right ] = \\
  = f(x) - 2 \sh{1} + 2 \sum_{k=2}^\infty\left [ \frac{\sqrt{2} \cdot
      \sh{1}\cdot (-1)^k}{\pi^2 k^2 + 1} \cdot \frac{\sqrt{2}\cos{\pi
        k x}}{\pi^2 k^2 - 1} \right ] = \\
  = f(x) + \sh{1} \left ( -2 + 4  \sum_{k=2}^\infty \frac{(-1)^k
