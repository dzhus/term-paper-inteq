...
Функция $\psi_1(x)$ называется \neword{собственной функцией}
интегрального оператора $\hat{I}(f) = \int_a^b{K(x, t) f(t)\, dt}$
(ядра $K(x, t)$), если
\begin{equation}
  \label{eq:eigfun}
  \psi_1(x) = \lambda_1 \intl_a^b{K(x, t) \psi(t)\, dt}
\end{equation}
При этом $\lambda_1$ называется \neword{характеристическим числом},
соответствующим собственной функции $\psi_1(x)$, а обратная величина
$\frac{1}{\lambda_1}$ — \neword{собственным числом}.
Характеристические числа симметричного ядра действительны.
Последовательность собственных функций симметричного ядра ортогональна
и её можно сделать ортонормированной.
