...
Теорема Гильберта—Шмидта гласит, что если функция $f(x)$ представима в
виде
\begin{equation}
  f(x) = \intl_a^b{K(x, t) g(t)\, dt}
\end{equation}
...
\begin{equation}
  \label{eq:gs-series}
  f(x) = \sum_{k=1}^{\infty}a_k\psi_k(x)
\end{equation}
где коэффициенты $a_k$ вычисляются следующим образом:
\begin{equation}
  a_k = \intl_a^b{f(x) \psi_k(x)\,dx} = \intl_a^b{\frac{g(x)}{\lambda_k}\psi_k(x)\,dx}
\end{equation}
...
функции $f(x)$, удовлетворяющей условиям теоремы.
...
С учётом \eqref{eq:eigfun} этот ряд преобразуем к виду
\begin{equation}
...
Известно, что предположение о представимости ядра $K(x, t)$ в
виде \label{eq:bilinear-kernel-series} верно, если оно симметрично,
непрерывно в квадрате $S={0\leq x \leq 1,\, 0 \leq t \leq 1}$ и имеет
в $S$ равномерно ограниченные частные производные.
...
Для решения краевой задачи рассмотрим три варианта значений
...
\item $\lambda > 1$
  ...
  \begin{equation*}
    C^2 \intl_0^1 {\cos^2{\pi k x}\, dx} = 2 \cdot \frac{1}{2} = 1
  \end{equation*}
...
С учётом полученных результатов заметим, что данное по условию
$\lambda = 2 \notin {\lambda_k}$.
